...
if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN is empty. Check /home/coworkingbot/.env")

GAS_WEBAPP_URL = os.getenv("GAS_WEBAPP_URL", "").strip()
API_TOKEN = os.getenv("API_TOKEN", "").strip()

_admin_ids_raw = os.getenv("ADMIN_IDS", "").strip()
ADMIN_IDS = [x.strip() for x in _admin_ids_raw.split(",") if x.strip()]

TZ_NAME = os.getenv("TZ", "Europe/Moscow").strip()
MOSCOW_TZ = pytz.timezone(TZ_NAME)

if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN пустой. Проверь /home/coworkingbot/.env")
if not GAS_WEBAPP_URL:
    raise RuntimeError("GAS_WEBAPP_URL пустой. Проверь /home/coworkingbot/.env")
if not API_TOKEN:
    raise RuntimeError("API_TOKEN пустой. Проверь /home/coworkingbot/.env")
if not ADMIN_IDS:
    raise RuntimeError("ADMIN_IDS пустой. Проверь /home/coworkingbot/.env")
...
def parse_date(date_str: str) -> tuple[datetime | None, str | None]:
...
async def call_google_script(action: str, payload: dict) -> dict:
    ...
    data = {"token": API_TOKEN, "action": action, **payload}
    ...
    try:
        ...
        asyncio.create_task(notify_admin_about_error(str(e), f"GAS action: {action}"))
...
async def get_report_from_gas(report_type: str, period: str = "current") -> dict:
    """Получение отчета"""
    result = await call_google_script("get_report", {"report_type": report_type, "period": period})
...
async def get_reviews_gas(
    public_only: bool = True, limit: int = 10, mask_names: bool = True
) -> dict:
    """Получение отзывов из GAS"""
    result = await call_google_script(
...
async def notify_admin_about_cancellation(
    record_id: str, booking_data: dict, user_id: int, reason: str = "пользователем"
):
...
async def notify_admin_about_payment_confirmation(record_id: str, client_name: str, admin_id: int):
    ...
    for admin in ADMIN_IDS:
        if int(admin) != admin_id:
            try:
                await bot.send_message(chat_id=int(admin), text=message_text, parse_mode="HTML")
            except Exception as e:
...
async def notify_admin_about_new_booking(booking_data: dict, record_id: str, user_id: int):
    ...
    confirm_button = InlineKeyboardButton(
        text="✅ Подтвердить оплату", callback_data=f"confirm_{record_id}"
    )
...
async def notify_admin_about_new_review(
    record_id: str, review_text: str, user_id: int, rating: int = 0
):
...
async def cmd_my_bookings(message: types.Message):
    ...
    result = await call_google_script(
        "get_user_bookings", {"user_id": user_id, "active_only": False}
    )
    ...
    if result.get("status") == "success":
        ...
        for i, booking in enumerate(bookings[:10], 1):
            ...
            if booking.get("status") == "Оплачено" and is_past_booking(booking.get("date")):
...
async def process_duration(message: types.Message, state: FSMContext):
    ...
    duration_map = {"1 час": 1, "2 часа": 2, "3 часа": 3, "4 часа": 4, "5 часов": 5, "6 часов": 6}
...
async def cmd_today_bookings(message: types.Message):
    ...
    if is_admin(user_id):
        ...
        result = await call_google_script(
            "get_user_bookings", {"user_id": user_id, "active_only": True}
        )
...
async def cmd_cancel(message: types.Message):
    ...
    # Отменяем бронь
    cancel_result = await call_google_script(
    ...
    if cancel_result.get("status") == "success":
        ...
        await notify_admin_about_cancellation(
            record_id, user_booking, message.from_user.id, "пользователем"
        )
...
async def cancel_booking_by_admin(message: types.Message, record_id: str, admin_id: int):
    ...
    booking_info = await call_google_script("get_booking_info", {"record_id": record_id})
...
async def notify_admin_about_cancellation(record_id: str, booking_data: dict, user_id: int):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(chat_id=int(admin_id), text=message_text, parse_mode="HTML")
        except Exception as e:
            logger.error(f"Ошибка отправки уведомления админу {admin_id}: {e}")
