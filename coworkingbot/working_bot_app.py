...
class Settings:
    ...
    admin_ids: list[str]
    tz_name: str
    moscow_tz: pytz.tzinfo.BaseTzInfo
...
async def send_admin_notification(text: str):
    ...
    for admin_id in ADMIN_IDS:
        try:
            if bot is None:
                logger.error("Bot is not initialized; cannot send admin notification.")
                return
...
async def main():
    ...
    try:
        if bot is None:
            logger.error("Bot is not initialized; aborting startup.")
            return
...
def _apply_settings(settings: Settings) -> None:
    global BOT_TOKEN, GAS_WEBAPP_URL, API_TOKEN, ADMIN_IDS, TZ_NAME, MOSCOW_TZ
    BOT_TOKEN = settings.bot_token
    GAS_WEBAPP_URL = settings.gas_webapp_url
